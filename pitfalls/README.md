...
| [`leptos-create-effect-inside-spawn-local-silent-gc`](leptos-create-effect-inside-spawn-local-silent-gc.md) | `create_effect` inside `spawn_local` future never re-runs, signal `set` fires but no subscriber reacts, page state never updates after async op, owner-less effect silently GC'd, LAN joiner stuck on WaitingForOpen, `wasm_bindgen_futures::spawn_local` no Leptos owner | fixed (holder pattern at component scope) |
...
| [`leptos-rwsignal-queue-self-clear-race`](leptos-rwsignal-queue-self-clear-race.md) | `ServerMsg`s randomly disappear, `Hello` dropped, host stuck on "Awaiting seat assignment", joiner board never renders post-Update, `RwSignal<Vec<T>>` queue with `set(Vec::new())` clear, drain-then-clear race, push between drain and clear silently lost | fixed (`Incoming` = VecDeque + monotonic tick signal) |
...
| [`wasm32-systemtime-now-panics`](wasm32-systemtime-now-panics.md) | "time not implemented on this platform" panic, `RuntimeError: unreachable` in browser console, `std::time::SystemTime::now()` on wasm32, `chess_net::room::now_ms` panic, in-browser action silently does nothing, chat Send does nothing, click handler chain crashes mid-execution | fixed (cfg-gate to `js_sys::Date::now()` on wasm32) |
...
| [`webrtc-set-remote-description-resolves-before-dc-open`](webrtc-set-remote-description-resolves-before-dc-open.md) | LAN joiner reaches "Connected" but board stays on "Waiting for server greeting…", Hello/ChatHistory silently lost on the wire, `dc.send_with_str` returns Err but result is discarded, `accept_answer().await` resolves before SCTP handshake completes, `dc.ready_state() == "connecting"` when host fanouts | fixed (`wait_for_dc_open` poll-based gate) |
